...
full_path_to_images = 'G:/공유 드라이브/화폐 이미지 데이터/THB'
...
for current_dir, dirs, files in os.walk('.'):
    ...
    for f in files:
        ...
        if f.endswith('.jpg') and os.path.isfile(f.replace('.jpg', '.txt')):
            print('file ', f, ' ', f.replace('.jpg', '.txt'))
            jpg_file_path = 'data/' + f
            if count%5 == 4:
                p_test.append(jpg_file_path + '\n')
            else:
                p.append(jpg_file_path + '\n')
...
with open(basepath + '/train.txt', 'w') as train_txt:
...
with open(basepath + '/test.txt', 'w') as test_txt:
    # Going through all elements of the list
    for e in p_test:
        # Writing current path at the end of the file
        test_txt.write(e)
